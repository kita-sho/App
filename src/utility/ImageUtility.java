...
public class ImageUtility extends Object
{
	...
	/**
	 * 画像(anImage)を赤成分のみの画像に変換した複製を応答する。
	 * @param anImage 画像(カラー)
	 * @return 画像(赤成分)
	 */
	public static BufferedImage redImage(BufferedImage anImage)
	{
		Integer width = anImage.getWidth();
		Integer height = anImage.getHeight();
		BufferedImage redImage = new BufferedImage(anImage.getWidth(), anImage.getHeight(), anImage.getType());
		ValueHolder<Integer> y = new ValueHolder<Integer>(0);
		new Condition(() -> y.get() < height).whileTrue(() ->
		{
			ValueHolder<Integer> x = new ValueHolder<Integer>(0);
			new Condition(() ->  x.get() < width).whileTrue(() ->
			{
				Integer aRGB = anImage.getRGB(x.get(), y.get());
				Double luminance = ColorUtility.luminanceFromRGB(aRGB);
				aRGB = ColorUtility.convertRGBtoINT(luminance, 0.0, 0.0);
				redImage.setRGB(x.get(), y.get(), aRGB);
				x.setDo((Integer it) -> it + 1);
			});
			y.setDo((Integer it) -> it + 1);
		});
	...
	/**
	 * 画像(anImage)を緑成分のみの画像に変換した複製を応答する。
	 * @param anImage 画像(カラー)
	 * @return 画像(緑成分)
	 */
	public static BufferedImage greenImage(BufferedImage anImage)
	{
		Integer width = anImage.getWidth();
		Integer height = anImage.getHeight();
		BufferedImage greenImage = new BufferedImage(anImage.getWidth(), anImage.getHeight(), anImage.getType());
		ValueHolder<Integer> y = new ValueHolder<Integer>(0);
		new Condition(() -> y.get() < height).whileTrue(() ->
		{
			ValueHolder<Integer> x = new ValueHolder<Integer>(0);
			new Condition(() ->  x.get() < width).whileTrue(() ->
			{
				Integer aRGB = anImage.getRGB(x.get(), y.get());
				Double luminance = ColorUtility.luminanceFromRGB(aRGB);
				aRGB = ColorUtility.convertRGBtoINT(0.0, luminance, 0.0);
				greenImage.setRGB(x.get(), y.get(), aRGB);
				x.setDo((Integer it) -> it + 1);
			});
			y.setDo((Integer it) -> it + 1);
		});
	...
	/**
	 * 画像(anImage)を青成分のみの画像に変換した複製を応答する。
	 * @param anImage 画像(カラー)
	 * @return 画像(青成分)
	 */
	public static BufferedImage blueImage(BufferedImage anImage)
	{
		Integer width = anImage.getWidth();
		Integer height = anImage.getHeight();
		BufferedImage blueImage = new BufferedImage(anImage.getWidth(), anImage.getHeight(), anImage.getType());
		ValueHolder<Integer> y = new ValueHolder<Integer>(0);
		new Condition(() -> y.get() < height).whileTrue(() ->
		{
			ValueHolder<Integer> x = new ValueHolder<Integer>(0);
			new Condition(() ->  x.get() < width).whileTrue(() ->
			{
				Integer aRGB = anImage.getRGB(x.get(), y.get());
				Double luminance = ColorUtility.luminanceFromRGB(aRGB);
				aRGB = ColorUtility.convertRGBtoINT(0.0, 0.0, luminance);
				blueImage.setRGB(x.get(), y.get(), aRGB);
				x.setDo((Integer it) -> it + 1);
			});
			y.setDo((Integer it) -> it + 1);
		});
	...
	{
		URL aURL = null;
		try { aURL = URI.create(aString).toURL(); }
